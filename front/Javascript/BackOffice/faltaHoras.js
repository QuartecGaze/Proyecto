...
function buildCard(f) {
  const socio = f.socio || {};
  const foto  = socio.foto ? `../../Recursos/FotosPerfil/${socio.foto}` : '../../Recursos/FotosPerfil/usuario.webp';
  ...
  div.innerHTML = `
    <div class="info-socio">
      <div class="foto-socio">
        <img src="${foto}" alt="Foto socio">
      </div>
      <div class="datos-socio">
        <h3>${safe(socio.nombre)} ${safe(socio.apellido)}</h3>
        <p>ID: ${safe(f.idPersona)}</p>
        <p>${safe(socio.direccion, 'Sin dirección')}</p>
      </div>
    </div>
